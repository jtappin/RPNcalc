% Note that though this looks like a LaTeX file, it won't actually
% build properly with normal latex or pdflatex. To produce a PostScript
% or PDF version, run:
%    latex2man rpncalc.tex rpncalc.1
% and then:
%    man -t ./rpncalc.1 | ps2pdf - - > rpncalc.pdf
\documentclass{article}

\usepackage{latex2man}

\begin{Name}{1}{rpncalc}{James Tappin}{RPN Scientific Calculator}{RPNcalc}
  \section{SYNOPSIS}

  \textbf{rpncalc} [\emph{option}]...
  ...
  \begin{description}
  \item [\textbf{-c}, \textbf{--closed}] start the
    calculator with the stack and memory display hidden.
  \item [\textbf{-o}, \textbf{--open}] start the calculator
    with the stack and memory display visible (this is the default).
  \item [\textbf{-r}, \textbf{--restore} \emph{ file}]
    Load the stack and memory contents from the given file.
  \item [\textbf{-m}, \textbf{--registers }\emph{ number}]
    Set the number of memory registers to create. The default is 10.
  \item [\textbf{-f}, \textbf{--focus}] Maintain keyboard
    focus in the input window.
  \item [\textbf{-R}, \textbf{--radians}] Set the trigonometry unit to
    radians.
  \item [\textbf{-D}, \textbf{--degrees}] Set the trigonometry unit to
    degrees.
  \item[\textbf{-G}, \textbf{--grads}] Set the trigonometry unit to grads
    (1/100 of a right angle).
  \item [\textbf{-h}, \textbf{--help}] Print a help text
    and exit.
  \end{description}
  ...
  Fortran~2003.
% \footnote{There is a perl script in the build system to
%     incorporate this manual into the help displayer.}.
...
\subsubsection{Hold Entry Focus:}
If this is enabled, then the input focus always snaps back to the entry
window after any operation.
